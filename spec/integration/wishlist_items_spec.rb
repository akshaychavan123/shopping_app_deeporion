  ...
  path '/api/v1/wishlists/{wishlist_id}/wishlist_items' do
    post 'Creates a wishlist item' do
      ...
      parameter name: :wishlist_item, in: :body, schema: {
        type: :object,
        properties: {
          product_item_variant_id: { type: :integer, description: 'ID of the product item variant' }
        },
      ...
      response '201', 'wishlist item created' do
        ...
        let(:product_item_variant_id) { create(:product_item_variant).id }
        let(:wishlist_item) { { product_item_variant_id: product_item_variant_id } }
      ...
      response '422', 'invalid params' do
        let(:wishlist_id) { create(:wishlist, user: user).id }
        let(:product_item_variant_id) { nil }
        let(:wishlist_item) { { product_item_variant_id: product_item_variant_id } }
